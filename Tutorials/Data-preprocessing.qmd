...
## 2-2. Pre-processing and merging

First, ensure values in the 'date' column in mobility is a real Date object
If it is already a Date, this will keep it as Date.
If it's a character like "2026-04-01", this will parse it correctly.
...
covidcases <- covidcases |>
  mutate(date = as.Date("2019-12-29") + 
           weeks(week - 1))
```
...
filtered_data <- merged_data |>
  filter(state %in% 
           c("California", "Florida", "Hawaii"))
```

## 3-1. Line graph: trends over time
...
m50trend <- ggplot(filtered_data, 
  ...
  geom_line() +
  labs(
    title = "Weekly Mobility Across 3 States",
    x = "Week number",
...
## 3-4. Bar graph: comparing states
...
We now have 'mean_m50' scores. We enter this to ggplot and its geom_col() function.
```{r}
ggplot(state_summary, aes(x = state, y = mean_m50)) +
  geom_col() +
  labs(
    title = "Average Mobility Across 3 States",
    x = "State",
    y = "Mean m50"
    )
```
...
:::callout-tip
- If you already summarized your data: then use the	geom_col(). 
- But if you want to use counts of rows: then use	geom_bar().
:::
