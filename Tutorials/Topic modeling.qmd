...
library(ldatuning)    # LDA: Selecting number of topics
```
...
```{r, warning=FALSE, message=FALSE}
news_corpus <- 
  read_csv("~/Desktop/covid_fournews.csv")
```
...
#View(df)
#write.csv(df, file = "~/Desktop/covidwithtopics.csv")
```
...
```{r, results="hide"}
news_corpus0 <- read_csv("~/Desktop/covid_fournews.csv")
```
